...
@router.post('/requests/users/{target_id}', response_model=FriendshipRead,
             responses={
...
async def send_request(
    ...
    """
    return service.create(target_id)


@router.patch('/requests/users/{target_id}', response_model=FriendshipRead,
              responses={
                  status.HTTP_404_NOT_FOUND: {
                      'model': DetailMessage
                  }
...
@router.delete('/requests/users/{target_id}', responses={
    status.HTTP_404_NOT_FOUND: {
        'model': DetailMessage
    }
